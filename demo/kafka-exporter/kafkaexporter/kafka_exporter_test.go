...
import (
	...
	"github.com/open-telemetry/opentelemetry-collector-contrib/exporter/kafkaexporter/model"
)
...
func TestTracesPusherJson(t *testing.T) {
	c := sarama.NewConfig()
	producer := mocks.NewSyncProducer(t, c)
	producer.ExpectSendMessageAndSucceed()

	log.Print("TestTracesPusherJson")
	p := kafkaTracesProducer{
		producer:  producer,
		marshaler: newPdataTracesMarshaler(model.NewJSONTracesMarshaler(), "otlp_json"),
	}
	t.Cleanup(func() {
		require.NoError(t, p.Close(context.Background()))
	})
...
func (e tracesErrorMarshaler) Marshal(_ pdata.Traces, _ string, _ map[string]string) ([]*sarama.ProducerMessage, error) {
	return nil, e.err
}
