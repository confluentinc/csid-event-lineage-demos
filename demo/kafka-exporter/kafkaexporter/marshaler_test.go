...
func TestOTLPTracesJsonMarshaling(t *testing.T) {
	...
	span.SetSpanID(pdata.NewSpanID([8]byte{0, 1, 2, 3, 4, 5, 6, 6}))
	span.SetParentSpanID(pdata.NewSpanID([8]byte{8, 9, 10, 11, 12, 13, 14}))
	...
	expectedJSON := []interface{}{
		map[string]interface{}{
			...
			"name":         t.Name(),
			"kind":         pdata.SpanKindInternal.String(),
			"startTime":    fmt.Sprint(now.UnixNano()),
			"endTime":      fmt.Sprint(now.Add(time.Second).UnixNano()),
		},
		map[string]interface{}{
			"traceId":      "0102030405060708090a0b0c0d0e0f10",
			"spanId":       "0001020304050605",
			"parentSpanId": "08090a0b0c0d0e00",
			"name":         t.Name(),
			"kind":         pdata.SpanKindInternal.String(),
			"startTime":    fmt.Sprint(now.UnixNano()),
