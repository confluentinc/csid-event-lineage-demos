...
type TracesMarshaler interface {
	...
	Marshal(traces pdata.Traces, topic string, span_map map[string]string) ([]*sarama.ProducerMessage, error)
...
func tracesMarshalers() map[string]TracesMarshaler {
	...
	otlpJSON := newPdataTracesMarshaler(model.NewJSONTracesMarshaler(), "otlp_json")
